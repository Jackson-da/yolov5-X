...
def run(
    ...
    # 创建自定义输出labels_2路径
    (save_dir / 'labels_2' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per images
            ...
            # 创建自定义输出labels_2路径
            txt_path2 = str(save_dir / 'labels_2' / p.stem) + ('' if dataset.mode == 'images' else f'_{frame}')  # im.txt
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        line2 = (cls, *xyxy, conf) if save_conf else (cls, *xyxy)  # labels format
